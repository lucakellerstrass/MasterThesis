...
public class DataSetCalibration {
	...
	/**
	 * Does nothing else than calibrating LIBOR Market Models using the given calibrations information
	 */
	public void calibrateLMMs() {
		...
		for(int i = 0 ; i < curveModelCalibrationMaschines.length; i++) {	
			...
			CalibrationMachineInterface Model1CalibrationMaschine = new LmmCalibrationMachine(numberOfPaths,
					3, calibrationInformations[i], curveModelCalibrationMaschines[i]);	
			// simulation machine
			try {
				...
				System.out.println("Creating the simulation model for the LMM in DataSetCalibration.calibrateLMMs() failed.");
				e.printStackTrace();
	...
	/**
	 * Does nothing else than calibrating Hull White Models using the given calibrations information
	 */
	public void calibrateHWs() {
		...
		for(int i = 0 ; i < curveModelCalibrationMaschines.length; i++) {	
			...
			EulerSchemeFromProcessModel process1 = new EulerSchemeFromProcessModel(brownianMotionM1,
					EulerSchemeFromProcessModel.Scheme.EULER);
			CalibrationMachineInterface Model1CalibrationMaschine = new HWCalibrationMachine(numberOfPaths,
					2, calibrationInformations[i], curveModelCalibrationMaschines[i]);	
			// simulation machine
			try {
				LIBORModelMonteCarloSimulationModel HwSimulationModel = Model1CalibrationMaschine
						.getLIBORModelMonteCarloSimulationModel(process1, false);
			} catch (SolverException e) {
				System.out.println("Creating the simulation model for the HWM in DataSetCalibration.calibrateHWs() failed.");
				e.printStackTrace();
			} catch (CalculationException e) {
				System.out.println("Creating the simulation model for the HWM in DataSetCalibration.calibrateHWs() failed.");
				e.printStackTrace();
