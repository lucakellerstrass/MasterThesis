...
/**
 * 
 * Implements (a numerical approximation of) the function \( (t,V) \mapsto E(
 * V(t) \vert \mathcal{F}_t ) \) where \( V(t) \) is the (sum of) discounted
 * future value(s) of an underlying \( V \), discounted to \( t \) and \( t \)
 * is a given evaluation time.
 *
 ...
 */
public class ExposureMachine extends AbstractProductComponent {
	...
	/**
	 * Creates (a numerical approximation of) the function \( (t,V) \mapsto E( V(t)
	 * \vert \mathcal{F}_t ) \) where \( V(t) \) is the (sum of) discounted future
	 * value(s) of an underlying \( V \), discounted to \( t \) and \( t \) is a
	 * given evaluation time.
	 *
	 * @param underlying The underlying.
	 */
	public ExposureMachine(AbstractLIBORMonteCarloProduct underlying) {
		super();
		this.underlying = underlying;
	}

	public AbstractLIBORMonteCarloProduct getUnterlying() {
		return underlying;
	 ...
	 * specified model, evaluated at a given evalutationTime. Note: For a lattice
	 * this is often the value conditional to evalutationTime, for a Monte-Carlo
	 * simulation this is the (sum of) value discounted to evaluation time.
	 * Cashflows prior evaluationTime are not considered. (very important for the
	 * exposure!!)
	 *
	...
	public RandomVariable getValue(double evaluationTime, LIBORModelMonteCarloSimulationModel model)
		...
		if (values.getFiltrationTime() > evaluationTime) {
			...
			MonteCarloConditionalExpectationRegression condExpEstimator = new MonteCarloConditionalExpectationRegression(
					filteredRegressionBasisFunctions, regressionBasisFunctions);

			// Calculate cond. expectation. Note that no discounting (numeraire division) is
			// required!
	 ...
	 *
	 * @param evaluationTime The date w.r.t. which the basis functions should be
	 *                       measurable.
	 ...
	 *                                                    method.
	 */
	private RandomVariable[] getRegressionBasisFunctions(double evaluationTime,
		...
		rate = model.getLIBOR(evaluationTime, model.getLiborPeriod(liborPeriodIndex),
				model.getLiborPeriod(liborPeriodIndexEnd));
	...
	/**
	 * This method prints out the exposure paths into the console
	 * 
	 * @param swapExposureEstimator
	 * @param simulationModel
	 * @param testSwap
	 * @throws CalculationException
	 */
	public static void printExpectedExposurePaths(TermStructureMonteCarloProduct swapExposureEstimator,
