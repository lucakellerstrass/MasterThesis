...
public class LMMCalibrationStorageTest {
	...
	public static void main(String[] args) throws SolverException, CalculationException {
		CallibrateAndStoreLMM();
		System.out.println("intitializatiion worked" + "\t");
		
		RestoreAndTestLMM();
		
		System.out.println("restoring worked" + "\t");
	...
	private static void RestoreAndTestLMM() throws SolverException, CalculationException {
		
		//Information we need to restore the LMM
...
final AnalyticModel curveModel = CurveModelCalibrationItem.getCalibratedCurve();
		...
		for (int j = 0; j < calibrationProducts.size(); j++) {
			AbstractLIBORMonteCarloProduct calibrationProduct = calibrationProducts.get(j).getProduct();
			try {
				double valueModel = calibrationProduct.getValue(LMMSimulation);
				double valueTarget = calibrationProducts.get(j).getTargetValue().getAverage();
				double error = valueModel-valueTarget;
				deviationSum += error;
				deviationSquaredSum += error*error;
				System.out.println(calibrationItemNames.get(j) + "\t" + "Model: " + formatterValue.format(valueModel) + "\t Target: " + formatterValue.format(valueTarget) + "\t Deviation: " + formatterDeviation.format(valueModel-valueTarget));// + "\t" + calibrationProduct.toString());
			}
			catch(Exception e) {
			}
		}
