...
public class CalibrationComparismForGUI {
	...
	public static void main(String[] args) throws SolverException, CalculationException {
		 ...
		 CalibrationInformation calibrationInformation1 = new CalibrationInformation(DataScope.RisingTerminals, DataSource.EXAMPLE );
		...
		//Co-Terminals
		 ...
		 CalibrationInformation calibrationInformation2 = new CalibrationInformation(DataScope.CoTerminals, DataSource.EXAMPLE );
		...
		//Full volatility surface
		 System.out.println("\n" + "Test of example Co-terminals calibration:");
		 CalibrationInformation calibrationInformation3 = new CalibrationInformation(DataScope.FullSurface, DataSource.EXAMPLE );
	...
	private static void Tester(CalibrationInformation calibrationInformation, boolean forcedCalculation) throws SolverException, CalculationException {
		
		//Initialization
		CalibrationMaschineInterface model1CalibrationMaschine = new LmmCalibrationMaschine(numberOfPaths, numberOfFactorsModel1, calibrationInformation);
		//Initialization
		CalibrationMaschineInterface model2CalibrationMaschine = new HWCalibrationMaschine(numberOfPaths, numberOfFactorsModel2, calibrationInformation);
		 ...
		 System.out.println("\n" + "Now we test how good the calibration worked for Model 1 and Model 2 and compared.");
			...
			LIBORModelMonteCarloSimulationModel model1SimulationModel = model1CalibrationMaschine.getLIBORModelMonteCarloSimulationModel(process1,forcedCalculation);
			LIBORModelMonteCarloSimulationModel model2simulationModel = model2CalibrationMaschine.getLIBORModelMonteCarloSimulationModel(process2,forcedCalculation);
			...
			// We get the calibration information from the "CalibrationInformation" instance
               ...
               System.out.println("The calculation of Model 1 took " + model1CalibrationMaschine.getCalculationDuration() + " milli seconds. Which are "+ formatterParam.format(model1CalibrationMaschine.getCalculationDuration()/60000) + " minutes." );
               System.out.println("The calculation of Model 2 took " + model2CalibrationMaschine.getCalculationDuration() + " milli seconds. Which are "+ formatterParam.format(model2CalibrationMaschine.getCalculationDuration()/60000) + " minutes." );
			...
			for (int i = 0; i < calibrationItems.length; i++) {
				...
				try {
					...
					System.out.println(model1CalibrationMaschine.getCalibrationItemNames(calibrationInformation)[i]
							+ "\t" + "Model 1: "+ "\t" + formatterValue.format(valueModel1) 
							+ "\t" + "Model 2: "+ "\t" + formatterValue.format(valueModel2) 
					        ...
					        + "\t Deviation Model 1 to Model 2: "+ "\t" + formatterDeviation.format(valueModel1-valueModel2));
			...
			System.out.println("RMS Error Model 1 ........: \t" + formatterDeviation.format(Math.sqrt(deviationSquaredSum1/calibrationItems.length)));
			System.out.println("Mean Deviation Model 2: \t" + formatterDeviation.format(averageDeviation2));
			System.out.println("RMS Error Model 2 ........: \t" + formatterDeviation.format(Math.sqrt(deviationSquaredSum2/calibrationItems.length)));
