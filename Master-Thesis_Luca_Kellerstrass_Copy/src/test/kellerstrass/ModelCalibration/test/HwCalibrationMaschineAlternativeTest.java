...
public class HwCalibrationMaschineAlternativeTest {
	...
	public static void main(String[] args) throws SolverException, CalculationException {
		...
		System.out.println("Second via the Calibration maschine intern calibration test methode");
				CalibrationMaschineInterface HwCalibrationMaschine = new HWCalibrationMaschineAlternative (numberOfPaths, numberOfFactors, calibrationInformation1);
	...
	private static void Tester(CalibrationInformation calibrationInformation, boolean forcedCalculation) throws SolverException, CalculationException {
		...
		//Initialization
		CalibrationMaschineInterface HwCalibrationMaschine = new HWCalibrationMaschineAlternative(numberOfPaths, numberOfFactors, calibrationInformation);
		 ...
		 System.out.println("The calculation took " + HwCalibrationMaschine.getCalculationDuration() + " milli seconds. Which are "+ formatterParam.format(HwCalibrationMaschine.getCalculationDuration()/60000) + " minutes." );
		 ...
		 System.out.println("\t" + "Now we test how good the calibration worked");
			...
			LIBORModelMonteCarloSimulationModel lIBORMarketModelSimulation = HwCalibrationMaschine.getLIBORModelMonteCarloSimulationModel(process,forcedCalculation);
			...
			for (int i = 0; i < calibrationItems.length; i++) {
				...
				try {
					...
					System.out.println(HwCalibrationMaschine.getCalibrationItemNames(calibrationInformation)[i] + "\t" + "Model: "+ "\t" + formatterValue.format(valueModel) + "\t Target: "+ "\t" + formatterValue.format(valueTarget) + "\t Deviation: "+ "\t" + formatterDeviation.format(valueModel-valueTarget));
				}
				catch(Exception e) {
				}
			}
