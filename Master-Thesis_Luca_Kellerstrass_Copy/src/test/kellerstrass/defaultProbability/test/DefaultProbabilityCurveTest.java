...
public class DefaultProbabilityCurveTest {
	...
	public void test() throws Exception {
		...
		double deltaT = 0.5; // 0.5; //0.05; //1.0
		...
		System.out.println("time  \t  default Prob until t ");

		double defaultProb = 0.0;
		System.out.print(0.0 + "   \t   " + 0.0);

		for (int i = 1; i <= timeDiscretization.getNumberOfTimes(); i++) {

			defaultProb += marginalDefaultProbsForwardBootstrap[i];
			
			double timeStart = timeDiscretization.getTime(i - 1);

			double timeEnd = timeDiscretization.getTime(i);

			System.out.print(formatterParam.format(timeEnd) + "   \t   " + defaultProb);
