...
/*+++++++Model input++++++++++++*/	
	...
	private static String paymentOffsetCode  = "3M";          //The Libor paymentOffsetCode
	private static double  discretizationPointPerPaymentIntervalll = 2.0;
	...
	private static double notional = 1.0; 
	private static Frequency fixedLegFrequanecy = Frequency.ANNUAL;
	private static Frequency floatingLegFrequanecy = Frequency.QUARTERLY;
	private static int StartInMonthsFromReferenceDate = 1;
	private static int SwapDurationInYears = 20;
	...
	public static void main(String[] args) throws Exception {
		...
		TermStructureMonteCarloProduct swapExposureEstimator1 = new ExposureEstimator(swap1);
		TermStructureMonteCarloProduct swapExposureEstimator2 = new ExposureEstimator(swap2);
 ...
 */
	private static AbstractLIBORMonteCarloProduct initSwap(LIBORModelMonteCarloSimulationModel simulationModel) {
		...
		LocalDate startDate = referenceDate.plusMonths(StartInMonthsFromReferenceDate);
		LocalDate endDate = startDate.plusYears(SwapDurationInYears);
...
double rate = 0.035;
		...
		AbstractIndex libor = new LIBORIndex(forwardCurve.getName(), "EUR", paymentOffsetCode, floatMetaSchedule.getBusinessdayCalendar(), floatMetaSchedule.getDateRollConvention());

		//Create legs
		TermStructureMonteCarloProduct floatLeg = new SwapLeg(floatLegSchedule, new Notional(notional), libor, 0, false);
		TermStructureMonteCarloProduct fixLeg = new SwapLeg(fixLegSchedule, new Notional(notional), null, rate, false);
 ...
 */
	private static void initModels() throws Exception {
		// Create the discount curve
				...
				double dt		= (liborPeriodLength / discretizationPointPerPaymentIntervalll); 
				double lastTime	= SwapDurationInYears + (StartInMonthsFromReferenceDate/12.0) + 1.0;
