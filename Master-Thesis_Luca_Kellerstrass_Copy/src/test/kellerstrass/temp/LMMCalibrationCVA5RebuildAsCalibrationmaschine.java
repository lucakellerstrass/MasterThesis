...
public class LMMCalibrationCVA5RebuildAsCalibrationmaschine {
	...
	public static void main(String[] args) throws Exception {
		...
		double dt = 0.25;
		CalibrationMaschineInterface lmmCalibrationMaschine = new LmmCalibrationMaschine(numberOfPaths, numberOfFactors, new CalibrationInformation(DataScope.CoTerminals, DataSource.EXAMPLE ));
		...
		//Now we test our CVA calculation	
		  final AnalyticModel curveModel = CurveModelCalibrationItem.getCalibratedCurve();
			CVA cvaGetter = new CVA(simulationModel, swap, Recovery, cdsSpreads, curveModel.getDiscountCurve("discountCurve-EUR"));
	...
	private static LIBORModelMonteCarloSimulationModel doATMSwaptionCalibration() throws SolverException, CalculationException {
		...
		System.out.println("Calibration of rate curves:     (calibration of the discount an forward rate via ATM swatps)");
                                  ...
                                  LmmCalibrationMaschine lmmCalibrationMaschine = new LmmCalibrationMaschine(numberOfPaths, numberOfFactors, calibrationInformation);		
				...
				LIBORModel liborMarketModelCalibrated = getCalibratedModel(lmmCalibrationMaschine,numberOfFactors,  numberOfPaths,  calibrationInformation ); 
				...
				try {
					...
					final ForwardCurve forwardCurve = curveModelNEW.getForwardCurve("ForwardCurveFromDiscountCurve(discountCurve-EUR,6M)");
				...
				for (int i = 0; i < calibrationProducts.length; i++) {
					...
					try {
						...
						System.out.println(lmmCalibrationMaschine.getCalibrationItemNames(calibrationInformation)[i] + "\t" + "Model: \t" + formatterValue.format(valueModel) + "\t Target: \t" + formatterValue.format(valueTarget) + "\t Deviation: \t" + formatterDeviation.format(valueModel-valueTarget));// + "\t" + calibrationProduct.toString());
					}
					catch(Exception e) {
					}
				}
				...
				System.out.println("RMS Error.....:" + formatterDeviation	.format(Math.sqrt(deviationSquaredSum/calibrationProducts.length)));
	...
	public static LIBORModel getCalibratedModel(LmmCalibrationMaschine lmmCalibrationMaschine, int numberOfFactors, int numberOfPaths, CalibrationInformation calibrationInformation ) {
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++		
		...
		double calibrationStart = System.currentTimeMillis();
				...
				properties.put("calibrationParameters", calibrationParameters);
			            	        ...
			            	        /*
				                     * We want to check if the calibration Products from the calibration maschine are correct
				                      */
				                      CalibrationProduct[] calibrationProducts = lmmCalibrationMaschine.getCalibrationProducts();
				...
				try {
					liborMarketModelCalibrated = new LIBORMarketModelFromCovarianceModel(
							liborPeriodDiscretization,
							curveModelNEW,
							forwardCurve, new DiscountCurveFromForwardCurve(forwardCurve),
							covarianceModelDisplaced,
							calibrationProducts /*calibrationItems*/,
							properties);
